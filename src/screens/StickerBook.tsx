...
export default function StickerBook() {
  ...
  return (
    <div className="screen flex items-center justify-center gap-8">
      <Starfield count={40} />
      <div className="absolute top-4 left-4 z-10">
        <BigButton size="md" icon="🗺️" color="bg-black/40" speakId={phraseId('btn_home')} onPress={() => go('map')} label="Till kartan" />
      </div>
      <div className="relative z-10 flex h-[78vh] w-[70vw] max-w-[900px] flex-wrap content-start gap-4 overflow-y-auto rounded-3xl bg-[#fff8e6] p-8 shadow-[0_12px_40px_rgba(0,0,0,0.5)] no-scrollbar" style={{ backgroundImage: 'repeating-linear-gradient(transparent 0 46px, #e6d9b8 46px 48px)' }}>
        {stickers.length === 0 && (
        ...
        {stickers.map((s, i) => (
          <motion.button
            ...
            className="big-emoji flex h-28 w-28 items-center justify-center rounded-2xl bg-white/70 text-[80px] shadow-md"
          >
            {s}
          </motion.button>
        ))}
